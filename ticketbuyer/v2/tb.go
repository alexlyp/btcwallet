...
func (tb *TB) buy(ctx context.Context, passphrase []byte, tip *chainhash.Hash) error {
	...
	// Set expiry to the last block of the sdiff interval.  When the next block
	// begins a new interval, use the final block of the next interval instead
	// of the final block if the current interval.
	var expiry int32
	if height+1 == nextIntervalStart {
		expiry = nextIntervalStart + intervalSize - 1
	} else {
		expiry = nextIntervalStart - 1
	}
